...
func TestPotKvs_Save(t *testing.T) {
	...
	t.Run("Save not empty KVS return valid swarm address", func(t *testing.T) {
 	...
 	t.Run("Put key-value pair a second time, then save - returns error", func(t *testing.T) {
		s, _ := pot.NewSwarmKvs(ls)
		err := s.Put(ctx, key1, val1)
		assert.NoError(t, err)
		err = s.Put(ctx, key1, val1)
 	...
 	t.Run("Save twice - ok", func(t *testing.T) {
		s, _ := pot.NewSwarmKvs(ls)
		err := s.Put(ctx, key1, val1)
		assert.NoError(t, err)
		_, err = s.Save(ctx)
	...
	t.Run("Change key's value between two saves - ok", func(t *testing.T) {
		s, _ := pot.NewSwarmKvs(ls)
		err := s.Put(ctx, key1, val1)
		assert.NoError(t, err)
		_, err = s.Save(ctx)
	...
	t.Run("Store same value under different key between two saves - ok", func(t *testing.T) {
		s, _ := pot.NewSwarmKvs(ls)
		err := s.Put(ctx, key1, val1)
		assert.NoError(t, err)
		_, err = s.Save(ctx)
	...
	t.Run("Put same key-value pair a second time between saves - returns error", func(t *testing.T) {
		s, _ := pot.NewSwarmKvs(ls)
		err := s.Put(ctx, key1, val1)
		assert.NoError(t, err)
		_, err = s.Save(ctx)
	...
	t.Run("Put same key-value pair a second time, on new-by-reference KVS, between saves - returns error", func(t *testing.T) {
		s, _ := pot.NewSwarmKvs(ls)
		err := s.Put(ctx, key1, val1)
		assert.NoError(t, err)
		ref, err := s.Save(ctx)
